...
func (d *SQL) getPath(k []string, o interface{}) (interface{}, bool) {
	...
	for thisKey, thisObj := range obj {
		if thisKey == k[0] {
			...
			if len(k) > 1 {
				objFinal, found := d.getPath(k[1:], thisObj)
				if found {
					return objFinal, found
				}
			} else {
...
func (d *SQL) delPath(k string, o interface{}) bool {
	...
	if len(keys) == 1 {
		...
		d.dropKeys()
		obj, exists := d.getPath(keys[:len(keys)-1], o)
		if !exists {
			return false
		}
...
func (d *SQL) get(k string, o interface{}) ([]string, error) {
	...
	for {
		...
		if found {
			...
			deleted := d.delPath(key, d.cache.v1)
			if !deleted {
				return d.Query.KeysFound, errors.New(DeleteObjKeyNotFound)
			}
			d.dropKeys()
		} else {
			break
